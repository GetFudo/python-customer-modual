...
ELASTIC_SEARCH_URL = "http://127.0.0.1"
DINE_STORE_CATEGORY_ID="650d5735254a6ac50f0fff44"
...
CHILD_PRODUCT_INDEX = "productindexnew"
index_products = CHILD_PRODUCT_INDEX
print(DINE_STORE_CATEGORY_ID)


# es=Elasticsearch([{'host':'http://127.0.0.1','scheme':'http','port':9200}])
...
stream = db.stores.find({"status":1},{"storeId": 1, "categoryId":1})
all_sttore =[]

def process_stream(product):
    # operationType = product["operationType"]
    product_id = str(product["storeId"])
    print("food product  ", product_id)
    find_store = db.childProducts.find_one({'storeId': ObjectId(product_id)}, {"storeId": 1, "storeCategoryId":1})
    if find_store is None:
        all_sttore.append(product_id)
        return 
...
total_tasks = 1000
num_threads = 2  # You can adjust this value based on your requirements

# Using ThreadPoolExecutor to manage threads
with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
    # Submit each product to be processed by a thread
    # This will asynchronously execute the process_stream function with each product
    # and return a future object for each submission
    futures = [executor.submit(process_stream, product) for product in stream]
    completed_count = 0
