...
def update_plp_data_from_plp(query_string, currency_code, search_type, store_category_id):
    time.sleep(5)
    '''code for the update the request in redis, if any changes made from admin then
                   it will update data in redis'''
    api_header = {
    ...
    request_url = "python/searchFilter?" + query_string + "&called=1"
    api_header['storeCategoryId'] = store_category_id
    expireOffer = requests.get(PYTHON_BASE_URL + request_url, headers=api_header)
    print(expireOffer.status_code)
    return True


def plp_page_update(product_id, user_id):
    api_header = {
        "Content-Type": "application/json",
        "Authorization": "Bearer eyJhbGciOiJSU0EtT0FFUCIsImN0eSI6IkpXVCIsImVuYyI6IkExMjhHQ00iLCJ0eXAiOiJKV1QifQ"
                         ".IW6kBQzeg3-MSxDUBWIk2OqaqHAA4N0vl4WD-5V-RJhrUY5aRXPt2QF"
        ...
        if product_details['storeId'] != "0":
            ...
            if store_details is not None:
                ...
                for main_category in product_details['categoryList']:
                    ...
                    if "childCategory" in main_category["parentCategory"]:
                        for child_category in main_category['parentCategory']['childCategory']:
                            ...
                            if hyperlocal == True and storelisting == True:
                                request_url = "python/searchFilter?&fname=" + first_category_name + "&page=1&s_id=" + str(
                                    product_details['storeId']) + "&sname=" + second_category_name + "&called=1&userId=" + str(user_id)
                                ...
                                for zone_id in store_details['serviceZones']:
                                    request_url = "python/searchFilter?&fname=" + first_category_name + "&page=1&z_id=" + str(
                                        zone_id['zoneId']) + "&sname=" + second_category_name + "&called=1&userId=" + str(user_id)
                                    expireOffer = requests.get(
                                        PYTHON_BASE_URL + request_url, headers=api_header
                                    )
                                    print(expireOffer.status_code)
                            else:
                                request_url = "python/searchFilter?&fname=" + first_category_name + "&page=1&sname=" + second_category_name + "&called=1&userId=" + str(user_id)
                                expireOffer = requests.get(
                                    PYTHON_BASE_URL + request_url, headers=api_header
                                )
                                print(expireOffer.status_code)
                    if hyperlocal == True and storelisting == True:
                        request_url = "python/searchFilter?&fname=" + first_category_name + "&page=1&s_id=" + str(
                            product_details['storeId']) + "&called=1&userId=" + str(user_id)
                        expireOffer = requests.get(PYTHON_BASE_URL + request_url, headers=api_header)
                        print(expireOffer.status_code)
                    elif hyperlocal == True and storelisting == False:
                        for zone_id in store_details['serviceZones']:
                            request_url = "python/searchFilter?&fname=" + first_category_name + "&page=1&z_id=" + str(
                                zone_id['zoneId']) + "&called=1&userId=" + str(user_id)
                            expireOffer = requests.get(PYTHON_BASE_URL + request_url, headers=api_header)
                            print(expireOffer.status_code)
                    else:
                        request_url = "python/searchFilter?&fname=" + first_category_name + "&page=1" + "&called=1&userId=" + str(user_id)
                        expireOffer = requests.get(PYTHON_BASE_URL + request_url, headers=api_header)
                        print(expireOffer.status_code)
        else:
            pass
